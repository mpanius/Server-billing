...
- Курсы валют ЦБ РФ с автоматическим обновлением.
...
Также установщик попробует автоматически получить курсы валют с официального XML endpoint ЦБ РФ. Если ЦБ недоступен, сервис стартует с `RUB:1`, а курсы можно обновить позже в разделе `Настройки`.
...
```text
RUB:1,USD:90.1234,EUR:98.1234
```

Для `USDT` используется курс `USD` к RUB, если отдельный курс `USDT` не указан вручную.
