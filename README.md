...
- Каталог провайдеров с поддержкой рекомендательных и реферальных ссылок.
...
В разделе `Провайдеры` есть каталог популярных VPS/VDS/cloud-провайдеров. Эти же шаблоны доступны в формах добавления сервера и аккаунта хостинга: выберите провайдера, и панель подставит домен, ссылку на кабинет, ссылку на оплату, период, валюту и заметку о том, где искать ID услуги.
...
У шаблона может быть поле `referral_url`. Если оно заполнено, кнопка `Открыть сайт` в каталоге будет вести по реферальной ссылке. Если поля нет, используется обычная `website_url`.
